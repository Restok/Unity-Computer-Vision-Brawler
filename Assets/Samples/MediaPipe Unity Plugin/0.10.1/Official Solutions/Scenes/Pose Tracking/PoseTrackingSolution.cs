...
namespace Mediapipe.Unity.PoseTracking
{
  public class PoseTrackingSolution : ImageSourceSolution<PoseTrackingGraph>
  {
    ...
    private Vector3 transformCameraCoordinates(NormalizedLandmark landmark, float xScale, float xOffset, float yScale, float yOffset)
    {
      return new Vector3((landmark.X) * xScale - xOffset, landmark.Y * yScale - yOffset);
